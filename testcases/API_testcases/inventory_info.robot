...
${data}          [{"part_no":"W25M02GVZEIdflk;", "supplier_part_no":"W25M02GVZEIG", "packing_name": "Tube", "warehouse_name": "Taichung", "qty": 12, "reserved_qty": 0}]
...
API test:post inventory Info
  ...
  ${header}=      create dictionary      Authorization=${token}    Content-Type=application/json
  ${data}         to json                ${data}
  ${resp}=        POST On Session        tdcv1                     /api/v1/product-inventory     headers=${header}     json=${data}    expected_status=400
  log to console  ${resp.json()}
